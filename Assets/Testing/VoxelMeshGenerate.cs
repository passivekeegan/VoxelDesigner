...
public class VoxelMeshGenerate : MonoBehaviour
{
	...
	public VoxelMapping voxelstyle;
	public List<Vector3Int> customvoxels;

	private ChunkMeshData _data;
	...
	private void UpdateChunkMesh(IJL chunk)
	{
		...
		for (int k = 0;k < voxels.Count;k++) {
			_data.UpdateVoxelData(voxels[k], new VoxelData(true, 1));
			...
			for (int cell_index = 0;cell_index < cnt;cell_index++) {
				...
				_data.UpdateVoxelData(voxels[k] + Vx.AdjBlockIJL[cell_index], new VoxelData(false, 1));
			}
		}
		_data.GenerateMesh();
	...
	private void AddCustomVoxels()
	{
		...
		for (int k = 0; k < customvoxels.Count; k++) {
			AddVoxelIJL(new	IJL(customvoxels[k].x, customvoxels[k].y, customvoxels[k].z));
		}
	}
