...
[System.Serializable]
public class RectMap : ComponentMap<RectDesign>
{
	[SerializeField]
	...
	private Dictionary<int, List<RectPattern>> _idpatterns;
	private Dictionary<RectPattern, int> _patternid;
	private Dictionary<RectPattern, RectPattern> _patternpattern;

	public RectMap()
	{
		_ids = new List<int>();
		_components = new List<RectDesign>();
		_pids = new List<int>();
...
#if UNITY_EDITOR
	public void AddPatternsToList(RectDesign component, List<RectPattern> list)
	{
	...
	protected override void ComponentDeleted(int id)
	{
		...
		if (Application.isPlaying) {
#endif
			List<RectPattern> patterns = _idpatterns[id];
