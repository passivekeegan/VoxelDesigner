...
[System.Serializable]
public class LongitudeMap : ComponentMap<LongitudeDesign>
{
	[SerializeField]
	...
	private Dictionary<int, List<LongitudePattern>> _idpatterns;
	private Dictionary<LongitudePattern, int> _patternid;
	private Dictionary<LongitudePattern, LongitudePattern> _patternpattern;

	public LongitudeMap()
	{
		_ids = new List<int>();
		_components = new List<LongitudeDesign>();
		_pids = new List<int>();
...
#if UNITY_EDITOR
	public void AddPatternsToList(LongitudeDesign component, List<LongitudePattern> list)
	{
	...
	protected override void ComponentDeleted(int id)
	{
		...
		if (Application.isPlaying) {
#endif
			List<LongitudePattern> patterns = _idpatterns[id];
